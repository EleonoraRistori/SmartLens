...
class Retrievor:
    ...
    def __load_compressor(self, compressor):
        ...
        details = [f[2] for f in features]
        artwork = [f[1] for f in features]
        matrix = [f[0] for f in features]
        self.matrix = np.array(matrix)
        self.artwork = np.array(artwork)
        self.details = np.array(details)
    ...
    def search(self, wanted, distance='cosinus', depth=1):
        ...
        return [
            self.artwork[nearest_ids].tolist(),
            self.details[nearest_ids].tolist(),
